...
async function exportUsers() {
  ...
  return rows.map((u) => ({
    ...
    conversation_count:   Number(u.conversation_count),
...
async function exportEvents() {
  /**
   * For each event:
   *   completed → attendee_count = users with status='attended' (real data)
   *   published → attendee_count = users with status='going'    (intent)
...
async function exportInteractions() {
   ...
   *   type_weight:  attended=1.0, going=0.7, member=0.9, conversation=0.85
  ...
  const rows = await client`
    -- attended (completed events) — strongest signal
    SELECT
      ea.user_id::text,
    ...
    WHERE ea.status = 'attended' AND e.status = 'completed'

    UNION ALL

    -- going (upcoming published events) — intent signal
    SELECT
      ea.user_id::text,
      ea.event_id::text                                                   AS item_id,
      'event'::text                                                       AS item_type,
    ...
    FROM event_attendees ea
    JOIN events e ON e.id = ea.event_id
    WHERE ea.status = 'going' AND e.status = 'published'
    ...
    SELECT
      c.initiator_id::text,
      c.recipient_id::text                                                AS item_id,
      'user'::text                                                        AS item_type,
      LEAST(1.0, GREATEST(0.05,
        0.85 * EXP(-EXTRACT(EPOCH FROM
          (NOW() - c.created_at)
        ) / ${HALF_LIFE_SECS})
      ))::float                                                           AS weight
    FROM conversations c WHERE c.status = 'active'

    UNION ALL

    SELECT
      c.recipient_id::text,
      c.initiator_id::text                                                AS item_id,
      'user'::text                                                        AS item_type,
      LEAST(1.0, GREATEST(0.05,
        0.85 * EXP(-EXTRACT(EPOCH FROM
          (NOW() - c.created_at)
