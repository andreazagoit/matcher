...
from ml.config import TRAINING_DATA_DIR, NEGATIVE_SAMPLES
from ml.features import (
    build_user_features,
    build_event_features,
    build_space_features,
    jaccard_tag_similarity,
)
...
def _build_cross_type_tag_edges(
    user_records: list[dict],
    item_records: list[dict],
    item_type: str,
    min_jaccard: float = 0.12,
    top_k_per_user: int = 8,
    max_users: int = 4_000,
    max_items: int = 2_000,
) -> list[dict]:
    """
    Weak cross-type supervision: user ↔ event (or user ↔ space) via tag overlap.

    The tag features are the natural semantic bridge between entity types — a user
    who weights "trekking" highly should be embedded near trekking events and spaces
    even without explicit interaction data.

    Each edge is bidirectional and weighted proportionally to the tag overlap, but
    capped at 0.35 so they stay weaker than real interaction edges (which reach 1.0).

    Complexity: O(n_users × n_items) — capped by max_users / max_items.
    """
    users = [u for u in user_records if u.get("id") and u.get("tag_weights")]
    items = [r for r in item_records if r.get("id") and r.get("tags")]
    ...
    item_tags_by_id: dict[str, set[str]] = {
        r["id"]: set(r["tags"]) for r in items
    }

    edges: list[dict] = []
    for u in users:
        uid = u["id"]
        tw = {t: float(w) for t, w in (u.get("tag_weights") or {}).items() if float(w) > 0}
        if not tw:
            continue

        scores: list[tuple[str, float]] = []
        for iid, itags in item_tags_by_id.items():
            jac = jaccard_tag_similarity(tw, itags)
            if jac >= min_jaccard:
                scores.append((iid, jac))

        scores.sort(key=lambda x: x[1], reverse=True)
        for iid, jac in scores[:top_k_per_user]:
            w = min(0.35, 0.1 + jac)   # weaker than real interactions
            edges.append({"anchor_type": "user",  "anchor_id": uid,
                          "item_type": item_type, "item_id": iid,  "weight": w})
            edges.append({"anchor_type": item_type, "anchor_id": iid,
                          "item_type": "user",    "item_id": uid,  "weight": w})
    return edges
...
def build_training_data(
    ...
    # Weak cross-type supervision via tag overlap.
    # Provides gradient for users/events/spaces that lack interaction data, and
    # crucially seeds the cross-type alignment from the very first epoch.
    all_positive_records.extend(
        _build_cross_type_tag_edges(users_raw, events_raw, item_type="event")
    )
    all_positive_records.extend(
        _build_cross_type_tag_edges(users_raw, spaces_raw, item_type="space")
    )
